...
Reproducibility:
  All experiments seed Python, NumPy, PyTorch (CPU + CUDA), and HuggingFace.
  Deterministic CUDA algorithms are enabled (with CUBLAS workspace pinned),
  so on the same hardware results are bit-identical across runs. On different
  hardware results will be very close but not necessarily byte-identical due
...
# ============================================================================
# DETERMINISM (must be set before any torch/CUDA work)
# ============================================================================

# Required for deterministic cuBLAS GEMM kernels.
os.environ.setdefault("CUBLAS_WORKSPACE_CONFIG", ":4096:8")
# Make Python hash randomisation deterministic across processes.
os.environ.setdefault("PYTHONHASHSEED", "0")
# Disable HF tokenizer parallelism, which can introduce non-determinism.
...
def set_seed(seed: int) -> None:
    ...
    On CUDA, also enables deterministic algorithms and disables cuDNN
    benchmark/non-deterministic kernels. With CUBLAS_WORKSPACE_CONFIG
    set at module load time, this gives bit-identical results across
    ...
    # HuggingFace Trainer respects this; helpful even when Trainer's own
    # `seed` arg is also set.
    try:
...
def train_logreg(train_texts, train_labels) -> LRArtifacts:
    ...
    # liblinear is deterministic given fixed data; random_state pins
    # tie-breaking just in case.
...
def train_distilbert(train_texts, train_labels, *, seed=42,
    ...
    args = TrainingArguments(
        ...
        seed=seed,
        data_seed=seed,
        dataloader_num_workers=0,    # 0 workers => deterministic batch order
        report_to=[],
    )
